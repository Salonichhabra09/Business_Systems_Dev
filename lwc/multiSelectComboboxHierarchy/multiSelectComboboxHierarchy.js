...
export default class MultiSelectComboboxHierarchy extends LightningElement {
    ...
    handleClick() {
        ...
        if (this.preSelectedOptions) {
            this.currentOptions.forEach(element => {
                if (this.preSelectedOptions.includes(element.label)) {
    ...
    @api checkValidity1() {
        ...
        if (this.required) {
            const inputField = this.template.querySelector('[data-id="custom-input"]');
            ...
            inputField.reportValidity(); // Trigger reportValidity() to show browser's built-in validation message
            //return false;
            //});
            //const inputField = this.template.querySelector('.input-val').value;
        }
        //return true;
