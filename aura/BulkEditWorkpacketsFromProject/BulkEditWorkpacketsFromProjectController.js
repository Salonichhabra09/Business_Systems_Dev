({
    doInit : function(component, event, helper) {
        ...
        var selectedids = myPageRef.state.c__listofWorkpackets;
        component.set("v.SelectedWorkpackets", selectedids);
        console.log("This is selected workpackets "+component.get("v.SelectedWorkpackets"));
	...
	/* addRow: function(component, event, helper) {
     ...
     clonerow: function(component, event, helper) {
        ...
        rowItemList.push({ 'sobjectType': 'Workpacket__c','CPS_Project_Number__c':component.get("v.ProjectId"),'Product_PS__c':JSON.stringify(docobj[index].Product_PS__c).slice(1, -1),'Solution_Type__c': JSON.stringify(docobj[index].Solution_Type__c).slice(1, -1),
                          'Status__c':JSON.stringify(docobj[index].Status__c).slice(1, -1),'Agreed_Delivery__c':JSON.stringify(docobj[index].Agreed_Delivery__c).slice(1, -1),'Description__c':JSON.stringify(docobj[index].Description__c).slice(1, -1),'CPS_Project_Manager__c':JSON.stringify(docobj[index].CPS_Project_Manager__c).slice(1, -1)});
